...
2. `login` reads the token from `--token`, piped stdin, or a hidden prompt, validates it with `GET /api/user/me` and stores it (config stays 0600; `--url` is stored too). `whoami` prints user, token name, scopes and resource restrictions; `logout` removes the stored token
...
`token` is optional (written by `login`, removed by `logout`, omitted when empty); the file is always saved with mode 0600, tightening an older, more permissive file. Environment overrides: `DOCSGPT_TOKEN`, `DOCSGPT_URL`.
